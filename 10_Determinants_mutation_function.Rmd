...
MPRA <- read_csv("~/Hsieh Lab/Project- MPRA/Twist Oligo Order.csv") %>% filter(Type=="WT") %>% select("ID")
```
...
##clean up cisbp file
#import motif file
cisbp <- read.delim("~/Hsieh Lab/Project- Computational/3'UTR Motif Mutation Analysis/Feb 2021 Lukas Results/cisbp_mutations.txt", header=FALSE)
...
##clean up eclip file
eclip <- read.delim("~/Hsieh Lab/Project- Computational/3'UTR Motif Mutation Analysis/Feb 2021 Lukas Results/eclip_mutations.txt", header=FALSE)
...
RBN <- read.delim("~/Hsieh Lab/Project- Computational/3'UTR Motif Mutation Analysis/Feb 2021 Lukas Results/RBN_mutations.txt", header=FALSE)
...
#miRNA
mirna <- read.delim("~/Hsieh Lab/Project- Computational/3'UTR Motif Mutation Analysis/Feb 2021 Lukas Results/mirna_mutations.txt", header=FALSE)
colnames(mirna) <- c("Gene","MutID","miRNA_In_Mutant","miRNA_In_WT")
mirna[ mirna == "[]" ] <- NA
mirna <- distinct(mirna)

#PAS
pas <- read.delim("~/Hsieh Lab/Project- Computational/3'UTR Motif Mutation Analysis/Feb 2021 Lukas Results/pas_mutations.txt", header=FALSE)
...
top_mirna <- read.delim("~/Hsieh Lab/Project- MPRA/Polysome Computational Analysis/Motif Analysis/Top_Expressed_miRNA.txt")
...
```{r}
deltaG <- read_delim("H:/hsiehlab/MAIN_DATA/UTR_Project_hg38/small_seq_on_plasmid_April_2021/rnafold/free_energy_minimum_Twist_order.txt", 
...
```{r}
seqs <- read.delim("H:/hsiehlab/MAIN_DATA/UTR_Project_hg38/Samantha_master_analysis/sequences/sequences_31_byhand.txt", stringsAsFactors=FALSE)
seqs_2 <- read.delim("H:/hsiehlab/MAIN_DATA/UTR_Project_hg38/Samantha_master_analysis/sequences/sequences_6861.txt", stringsAsFactors=FALSE)
...
polysome <-read_csv("~/Hsieh Lab/Project- MPRA/Polysome Computational Analysis/All_Polysome_MPRA_results.csv")[,-1]
...
IVT <- read_csv("~/Hsieh Lab/Project- MPRA/4sU-IVT MPRA Analysis (Actual)/Read_count_filtered/IVT_stats_9-19.csv")[,-1]
sig_IVT <- IVT %>% 
